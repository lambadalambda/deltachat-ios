...
public class KeychainManager {
    ...
    private static var sharedKeychainGroup: String? {
        DcSharedContainer.sharedKeychainAccessGroup
    }
    ...
    public static func deleteDBSecrets() -> Bool {
        ...
        if let group = KcM.sharedKeychainGroup {
    ...
    private static func addAccountSecret(id: Int) throws -> String {
        var keychainItemQuery: [String: Any] = [
            kSecValueData as String: createRandomPassword().data(using: .utf8)!,
            kSecAttrAccount as String: "\(id)",
            kSecClass as String: kSecClassGenericPassword,
            kSecAttrAccessible as String: kSecAttrAccessibleAfterFirstUnlock
        ]
        if let group = KcM.sharedKeychainGroup {
            keychainItemQuery[kSecAttrAccessGroup as String] = group
        }
